...
TbsGrid.prototype.tbs_createFrame = function () {
    ...
    s += '  		</div>';
    s += '  		<div class="tbs-grid-horizontal-scroll2-left"><div>◀</div></div>';
    s += '  		<div class="tbs-grid-horizontal-scroll2-right"><div>▶</div></div>';
    s += '  	</div>';
    ...
    s += '	</div>';
    s += '  <div class="tbs-grid-layer" style="left:30000px;"></div>';
    s += '</div>';
...
TbsGrid.prototype.tbs_createTable40 = function () {
    ...
    let getColName = function (col, topColumns) {
        ...
        for (let x = 0; x < len; x++) { if (topColumns[x][grid.column_name] == col[grid.column_name] && topColumns[x][grid.column_type] == 'string') return topColumns[x][grid.column_text]; }
        return '';
    };
    for (let i = 0; i < len; i++) {
        let col = column[i];
    ...
    for (let i = srow; i <= erow; i++){
        for (let colIndex = 0; colIndex < this.columns.length; colIndex++) {
            ...
            if (col[grid.column_summaryType] != '') {
                ...
                row.layout[col[grid.column_name]][grid.layout_text] = this.tbs_renderText(col, row.data[col[grid.column_name]]);
            }
...
TbsGrid.prototype.setGroup = function (sortColumns, summaryColumns, mergeType) {
    ...
    //=============================================================
    grid.tbs_setScroll(grid.code_horizontal);
    grid.tbs_setScroll(grid.code_vertical);
        ...
        for (let x = 0, len2 = grid.columns.length; x < len2; x++){
            if (grid.headerColumns[i][x][grid.column_kind] == 'column') {
                let width = parseInt(grid.getTextWidth(canvas
                                            , grid.headerColumns[i][x][grid.column_text], fontSize, fontFamilty));
                if (width >= arr[x]) {
                    arr[x] = width;
                }
            }

...
TbsGrid.prototype.tbs_displayPanel30_merge = function (topRowIndex) {
    ...
    for (let ri = topRowIndex; ri < count; ri++) {
        ...
        for (let x = 0; x <= lastColumnIndex; x++) {
            ...
            let colValue = this.tbs_getLayoutData(ri, column[grid.column_name], this.layout_text);
...
TbsGrid.prototype.tbs_displaySelectedLineFixCol = function() {
    ...
    if (activefixBottom && activeContent) {
        ...
        if (this.headerRowCount > 1) {
            for (let i = this.headerRowCount - 1; i >= 0; i--) {
                if (trFixTopList[i].childNodes[colArray[0]].dataset.name != '') {
                    startCellRect = trFixTopList[i].childNodes[colArray[0]].getBoundingClientRect();
                    break;
                }
            }
            for (let i = this.headerRowCount - 1; i >= 0; i--) {
                if (trHeaderList[i].childNodes[colArray[colArray.length - 1]].dataset.name != '') {
    ...
    //============================================================= else if (activefixBottom) {
        ...
        if (this.headerRowCount > 1) {
            for (let i = this.headerRowCount - 1; i >= 0; i--) {
                if (trHeaderList[i].childNodes[colArray[0]].dataset.name != '') {
...
TbsGrid.prototype.tbs_displayHeaderDataFixCol2 = function() {
    ...
    for (let i = 0, rowLen = trList.length; i < rowLen; i++) {
        ...
        for (let x = 0, colLen = fixedColumnIndex; x < colLen; x++) {
            ...
            //=============================================================	 init cell
            cell.style.textAlign = 'center';
            cell.dataset.name = (col[grid.column_name] == undefined || col[grid.column_name] == '') ? '' : col[grid.column_name];
            cell.colSpan = 1;
            cell.rowSpan = 1;
            //=============================================================	 init cell
            if (col[grid.column_name] != undefined && col[grid.column_name] != '') {
                if (i == 0) cell.rowSpan = rowLen;
                else if (i < rowLen - 1) cell.rowSpan = rowLen - i;
                cell.dataset.name = col[grid.column_name];
                cell.style.display = '';
...
TbsGrid.prototype.tbs_setDataTable2 = function (param) {
    ...
    for (let i = topRowIndex; i < bottomRowIndex + 1; i++) {
        ...
        for (let x = 0; x <= lastColumnIndex; x++) {
            ...
            let colValue = this.tbs_getLayoutData(i, column[grid.column_name], this.layout_text);
    ...
    if (this.fixedColumnIndex != -1){
        ...
        for (let i = 0, rowLen = tablesRows.length; i < rowLen; i++) {
            ...
            for (let x = 0, colLen = this.columns.length; x < colLen; x++) {
                ...
                tableCell.dataset.name = (header[grid.column_kind] == 'column') ? header[grid.column_name] : '';
...
TbsGrid.prototype.tbs_setDataSummaryTable = function (param) {
    ...
    //=============================================================
    let summaryRow = JSON.parse(JSON.stringify(this.data_table[0].data));
    for (let i = 0, colCount = summaryColumns.length; i < colCount; i++) {
        let col = summaryColumns[i];
        if (col[grid.column_type] == 'number') {
            summaryRow[col[grid.column_name]] = 0;
        }
    }
    //============================================================= summary
    for (let rowIndex = 0, len = this.data_view.length; rowIndex < len; rowIndex++) {
        let row = this.data_view[rowIndex].data;
        for (let i = 0, colCount = summaryColumns.length; i < colCount; i++) {
            let column = summaryColumns[i];
            if (column[grid.column_type] == 'number') {
                let columnName = column[grid.column_name];
                summaryRow[columnName] += parseFloat(row[columnName]);
            }
        }
    }
    //============================================================= summary : sum, avg calculate
    let rowCount = this.data_view.length;
    for (let i = 0, colCount = summaryColumns.length; i < colCount; i++) {
        let column = summaryColumns[i];
        if (column[grid.column_type] == 'number' && column[grid.column_summaryType] == 'avg') {
            let columnName = column[grid.column_name];
            summaryRow[columnName] = summaryRow[columnName] / rowCount;
        }
    }
    //============================================================= display Data
    ...
    for (let i = 0, len = this.columns.length; i < len; i++) {
        ...
        for (let x = 0, len2 = summaryColumns.length; x < len2; x++){
            ...
            if (columnName == summaryColumn[grid.column_name]) {
                if (column[grid.column_type] == 'number') {
                    cell.childNodes[0].textContent = this.tbs_render(column, summaryRow[columnName]).text;
                    break;
                }
                else {

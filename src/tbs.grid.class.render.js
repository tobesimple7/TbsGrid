...
class TbsGridRender {
    ...
    init(panelName, tableCell, column, rowIndex, columnIndex) {
    ...
    start(panelName, tableCell, column, rowIndex, columnIndex) {
        ...
        if (this.panelName == 'panel40' || this.panelName == 'panel42') {
            let topColumn   = grid.classTop.getTopColumn(this.columnName);
            if (grid.notNull(topColumn)) {
                ...
                this.className  = grid.notNull(topColumn[grid.column_className]) ? topColumn[grid.column_className] : grid.classColumn.getColumnProperty(this.columnName, grid.column_className);
            }
            else {
                this.align      = column[grid.column_align];
                this.className  = grid.classColumn.getColumnProperty(this.columnName, grid.column_className);
            }
            this.cellValue  = grid.classTop.getTopValue(this.rowIndex, this.columnName);
            this.cellText   = grid.classTop.getTopText(this.rowIndex, this.columnName);
        }
        else if (this.panelName == 'panel50' || this.panelName == 'panel52') {
            let footerColumn   = grid.classFooter.getFooterColumn(this.columnName);
            if (grid.notNull(footerColumn)) {
                ...
                this.className  = grid.notNull(footerColumn[grid.column_className]) ? footerColumn[grid.column_className] : grid.classColumn.getColumnProperty(this.columnName, grid.column_className);
            }
            else {
                this.align      = column[grid.column_align];
                this.className  = grid.classColumn.getColumnProperty(this.columnName, grid.column_className);
            }
            this.cellValue  = grid.classFooter.getFooterValue(this.rowIndex, this.columnName);
            this.cellText   = grid.classFooter.getFooterText(this.rowIndex, this.columnName);
        }
        else {
            this.align      = column[grid.column_align];
            this.className  = grid.classColumn.getColumnProperty(this.columnName, grid.column_className);
    ...
    setTableRow(tableRow, rowIndex, panelName = 'panel30') {
        let selector = this.selector;
        let grid = this.grid;
        let classRender = this;
    ...
    showAlternativeRowColor(panelName, tableRow, rowIndex) {
        let selector = this.selector;
        let grid = this.grid;
        let classRender = this;
        return;
    ...
    hideTableRows(panelName, tableRows, fromRowIndex, toRowIndex) {
        let selector = this.selector;
        let grid = this.grid;
        let classRender = this;
    ...
    hideTableCells(panelName, tableRow, lastColumnIndex) {
        let selector = this.selector;
        let grid = this.grid;
        let classRender = this;
    ...
    showSelectedCells(panelName, tableCell, rowIndex, cellIndex) {
        let selector = this.selector;
        let grid = this.grid;
        let classRender = this;

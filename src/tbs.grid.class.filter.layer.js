class TbsGridFilterLayer {
    ...
    getComboData() {
        ...
        if (column[grid.column_type] == grid.code_string) {
            let option;
            gridFilter.addComboOption(combo, grid.const_filterEqual         , grid.getCofnigLabel('filter_equal'));
            gridFilter.addComboOption(combo, grid.const_filterInclude 		, grid.getCofnigLabel('filter_include'));
            gridFilter.addComboOption(combo, grid.const_filterStartCharacter, grid.getCofnigLabel('filter_startCharacter'));
            gridFilter.addComboOption(combo, grid.const_filterEndCharacter	, grid.getCofnigLabel('filter_endCharacter'));
            gridFilter.addComboOption(combo, grid.const_filterBlank         , grid.getCofnigLabel('filter_blank'));
            gridFilter.addComboOption(combo, grid.const_filterNotEqual      , grid.getCofnigLabel('filter_notEqual'));
            gridFilter.addComboOption(combo, grid.const_filterNotInclude 	, grid.getCofnigLabel('filter_notInclude'));
        }
        else if (column[grid.column_type] == grid.code_number || column[grid.column_type] == grid.code_currency) {
            let option;
            gridFilter.addComboOption(combo, grid.const_filterSelect      , grid.getCofnigLabel('filter_select'));
            gridFilter.addComboOption(combo, grid.const_filterEqual       , grid.getCofnigLabel('filter_equal'));
            gridFilter.addComboOption(combo, grid.const_filterGreaterEqual, grid.getCofnigLabel('filter_greaterEqual'));
            gridFilter.addComboOption(combo, grid.const_filterLessEqual   , grid.getCofnigLabel('filter_lessEqual'));
            gridFilter.addComboOption(combo, grid.const_filterGreater     , grid.getCofnigLabel('filter_greater'));
            gridFilter.addComboOption(combo, grid.const_filterLess        , grid.getCofnigLabel('filter_less'));
            gridFilter.addComboOption(combo, grid.const_filterBetween     , grid.getCofnigLabel('filter_between'));
            gridFilter.addComboOption(combo, grid.const_filterBlank       , grid.getCofnigLabel('filter_blank'));
        }
        return combo;
    }
